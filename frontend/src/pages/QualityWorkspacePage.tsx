...
function DeadlinesView(){const { processes }=useQualityProcesses();return <SimplePage title="Termíny a workflow" eyebrow="PROCESNÍ ŘÍZENÍ"><DataTable headers={['Proces','Aktuální krok','Odpovědná osoba','Termín','Stav']} rows={processes.map((process)=>{const step=process.workflow[process.currentStepIndex];return [process.title,step?.title??'—',demoPersonas.find((p)=>p.id===step?.personaId)?.name??'—',step?.deadline??'—',process.status==='COMPLETED'?'Dokončeno':'Aktivní']})}/></SimplePage>}
...
function RvhView(){const { processes }=useQualityProcesses();return <SimplePage title="Podklady pro RVH" eyebrow="ROZHODOVACÍ PODPORA"><DataTable headers={['Proces','Typ','Stav','Poslední krok']} rows={processes.map((p)=>[p.title,p.processType,p.status,p.workflow[p.currentStepIndex]?.title??'—'])}/></SimplePage>}
...
function ArchiveView({ externalReadOnly = false }: { externalReadOnly?: boolean }) {
  ...
  const historical = [
    { faculty:'FSI', programme:'N-PI', name:'Procesní inženýrství', kind:'Akreditace / RVH', accreditation:'2025', evaluation:'2025', status:'Uzavřeno', decision:'Schváleno', id:'ARCH-NPI-2025' },
    { faculty:'FSI', programme:'N-PI', name:'Procesní inženýrství', kind:'Roční evaluace', accreditation:'2025', evaluation:'2026', status:'Uzavřeno', decision:'3 opatření', id:'ARCH-NPI-2026' },
    { faculty:'FSI', programme:'N-PI', name:'Procesní inženýrství', kind:'Roční evaluace', accreditation:'2025', evaluation:'2027', status:'Uzavřeno', decision:'2 opatření', id:'ARCH-NPI-2027' },
    { faculty:'FIT', programme:'N-IT', name:'Informační technologie', kind:'Periodické hodnocení', accreditation:'2024', evaluation:'2027', status:'Uzavřeno', decision:'Bez zásadních zjištění', id:'ARCH-NIT-2027' },
  ]
  const completed = processes.filter((p)=>p.status==='COMPLETED').map((p)=>({faculty:p.scope.includes('FSI')?'FSI':'VUT',programme:p.programmeCode,name:p.title,kind:p.processType,accreditation:'2027',evaluation:'2027',status:'Uzavřeno',decision:'Dokončený proces',id:p.id}))
  const records=[...historical,...completed].filter((r)=>(faculty==='Všechny'||r.faculty===faculty)&&(programme==='Všechny'||r.programme===programme)&&(year==='Všechny'||r.evaluation===year))
  return <div className="content-frame quality-office-page"><PageHeading eyebrow={externalReadOnly?'EXTERNÍ PŘÍSTUP · POUZE PRO ČTENÍ':'ŘÍZENÍ KVALITY · INSTITUCIONÁLNÍ PAMĚŤ'} title="Archiv hodnocení studijních programů" description="Neměnné snapshoty uzavřených akreditací, evaluací, formulářů, dokumentů, rozhodnutí a auditní stopy."/>{externalReadOnly&&<div className="archive-readonly-banner"><strong>Režim pouze pro čtení</strong><span>Vidíte pouze archivní záznamy zpřístupněné VUT pro externí evaluaci. Aktivní procesy ani interní pracovní komentáře nejsou dostupné.</span></div>}<section className="quality-office-panel archive-filters"><label>Fakulta<select value={faculty} onChange={(e)=>setFaculty(e.target.value)}><option>Všechny</option><option>FSI</option><option>FIT</option></select></label><label>Studijní program<select value={programme} onChange={(e)=>setProgramme(e.target.value)}><option>Všechny</option><option>N-PI</option><option>N-IT</option></select></label><label>Rok evaluace<select value={year} onChange={(e)=>setYear(e.target.value)}><option>Všechny</option><option>2025</option><option>2026</option><option>2027</option></select></label><label>Rok akreditace<select><option>Všechny</option><option>2024</option><option>2025</option><option>2027</option></select></label></section><section className="quality-office-panel"><div className="archive-table-head"><div><h2>Archivní záznamy</h2><p>{records.length} záznamů odpovídá zvoleným filtrům.</p></div>{!externalReadOnly&&<button className="vut-secondary-button" type="button">+ Připravit balíček pro evaluátora</button>}</div><DataTable headers={['Fakulta','Program','Typ záznamu','Akreditace','Evaluace','Stav / výsledek']} rows={records.map((r)=>[r.faculty,`${r.programme} · ${r.name}`,r.kind,r.accreditation,r.evaluation,`${r.status} · ${r.decision}`])}/></section><section className="quality-office-panel"><h2>Rozhodnutí a platnost z evidovaných procesů</h2><DataTable headers={['Program','Proces','Rozhodnutí','Platnost','Dokument']} rows={processes.flatMap((p)=>(p.decisionHistory??(p.decision?[p.decision]:[])).map((d)=>[p.programmeCode,p.title,d.resolutionNumber||'—',`${d.validFrom||'—'} – ${d.validTo||'—'}`,d.documentName||'—']))}/></section><section className="quality-office-panel archive-snapshot"><h2>Obsah archivního snapshotu</h2><div className="archive-content-grid"><span>Formuláře a odpovědi</span><span>Dokumenty a důkazy</span><span>Stanoviska a připomínky</span><span>Rozhodnutí RVH</span><span>Zjištění a opatření</span><span>Auditní stopa</span></div><p>Archivní verze je neměnná. Oprava nebo nové hodnocení vytváří nový navazující záznam, původní historie zůstává zachována.</p></section></div>
}
